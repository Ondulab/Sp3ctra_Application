...
int display_Init(sfRenderWindow *window) {
  // Si la fenêtre est NULL, c'est parce que nous utilisons Qt au lieu de SFML
  // On ne génère pas d'erreur dans ce cas
  if (!window) {
    printf("Using Qt renderer instead of SFML\n");
    return 0;
  }
...
void printImageRGB(sfRenderWindow *window, uint8_t *buffer_R, uint8_t *buffer_G,
  ...
  // Si la fenêtre est NULL, on n'affiche rien (Qt est utilisé à la place)
  if (!window) {
    return;
  }
